...
AppleAPI::AppleAPI() : _authClient(U("https://idmsa.apple.com")), _client("https://developerservices2.apple.com/services/" + kProtocolVersion)
{
...
pplx::task<std::shared_ptr<Account>> AppleAPI::Authenticate(std::string appleID, std::string password)
{
    ...
    auto encodedAppleID = web::uri::encode_uri(appleID, uri::components::fragment);
    auto encodedPassword = web::uri::encode_uri(password, uri::components::fragment);
    
    std::stringstream ss;
    ss << "format=" << "plist";
    ss << "&appIdKey=" << kAppIDKey;
    ss << "&appleId=" << encodedAppleID;
    ...
    .then([=](string_t plistXML)
          {
              ...
              plist_from_xml(plistXML.c_str(), (int)plistXML.size(), &plist);
                                              ...
                                              std::shared_ptr<Team> team)
{
    ...
    builder.append_query("clientId", kClientID);
    
    http_request request(methods::POST);
    request.set_request_uri(builder.to_string());
    request.set_body(plistXML);
    
    std::string cookie = "myacinfo=" + account->cookie();
